...
contract Game {
    ...
    /// @param _bet This is an obscured 32-byte string produced after hashing (real_bet || salt)
    ...
    /// @notice Receives the players real bets and their salt and check the initial bet validity
    /// @param _realBet A string representing the real bet
    /// @param _salt The salt that the message sender used to create his initial obscured bet
    ...
    /// @notice Let a player to stop the game and get refund in case his opponent griefs
    function requestRefund() external{
        // Checks
        require(block.timestamp > _playDeadline && msg.sender == _playedLast && _winner == address(0), "You are not allowed  to request a refund yet!");
    ...
    /// @notice Send money to the winner or the refund requestor and reset game variables for a new round
